...
//  - logos: a list of logos to use instead of the European Commission logo, each of which being defined by:
...
//  - additionalCopyrightHolders: a copyright owner in addition to European Union (or the only one if specStatus
//      is unofficial)
...
export const name = "semiceu/headers";
...
//    }-${conf.shortName}-${concatDate(conf.publishDate)}/`;
    conf.thisVersion = conf.thisVersionURI;
...
//    conf.latestVersion = `https://www.w3.org/${publishSpace}/${conf.shortName}/`;
    conf.latestVersion = conf.latestVersionURI;
...
//  if (conf.previousPublishDate) {
  if (conf.previousPublishDate || conf.prevVersionURI) {
//    if (!conf.previousMaturity && !conf.isTagFinding) {
    if (!(conf.previousMaturity || conf.prevVersionURI) && !conf.isTagFinding) {
...
//      )}`;
      conf.prevVersion = conf.prevVersionURI;
    } else if (conf.isCGBG) {
...
//      }-${concatDate(conf.previousPublishDate)}/`;
  ...
  conf.prependSEMIC = !conf.isUnofficial;
